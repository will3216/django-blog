...
urlpatterns = patterns('blog.views',
    ...
    url(r'^overview/$', ListView.as_view(
        queryset=Post.objects.filter(tags__name__in=["overview"]).order_by("order"),
        template_name="overview.html")),
    url(r'^technical_drawings/$', ListView.as_view(
        queryset=Post.objects.filter(tags__name__in=["technical_drawings"]).order_by("order"),
        template_name="technical_drawings.html")),
    url(r'^safety/$', ListView.as_view(
        queryset=Post.objects.filter(tags__name__in=["safety"]).order_by("order"),
        template_name="safety.html")),
    url(r'^measuring/$', ListView.as_view(
        queryset=Post.objects.filter(tags__name__in=["measuring"]).order_by("order"),
        template_name="measuring.html")),
    url(r'^materials/$', ListView.as_view(
        queryset=Post.objects.filter(tags__name__in=["materials"]).order_by("order"),
        template_name="materials.html")),
    url(r'^machining_and_tools/$', ListView.as_view(
        queryset=Post.objects.filter(tags__name__in=["machining_and_tools"]).order_by("order"),
    ...
    url(r'^about/$', ListView.as_view(
        queryset=Post.objects.filter(tags__name__in=["about"]).order_by("order"),
